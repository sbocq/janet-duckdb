...
(test (with [db (db/open)]
  ...
  @{"access_mode" "Access mode of the database (AUTOMATIC, READ_ONLY or READ_WRITE)"
    ...
    "disable_database_invalidation" "Disables invalidating the database instance when encountering a fatal error. Should be used with great care, as DuckDB cannot guarantee correct behavior after a fatal error."
    ...
    "enable_http_logging" "(deprecated) Enables HTTP logging"
    ...
    "experimental_metadata_reuse" "EXPERIMENTAL: Re-use row group and table metadata when checkpointing."
    ...
    "http_logging_output" "(deprecated) The file to which HTTP logging output should be saved, or empty to print to the terminal"
    ...
    "logging_mode" "Determines which types of log messages are logged"
    "logging_storage" "Set the logging storage (memory/stdout/file/<custom>)"
    ...
    "merge_join_threshold" "The maximum number of rows on either table to choose a merge join"
    ...
    "nested_loop_join_threshold" "The maximum number of rows on either table to choose a nested loop join"
    ...
    "pin_threads" "Whether to pin threads to cores (Linux only, default AUTO: on when there are more than 64 cores)"
    ...
    "produce_arrow_string_view" "Whether Arrow strings should be produced by DuckDB in Utf8View format instead of Utf8"
    "profile_output" "The file to which profile output should be saved, or empty to print to the terminal"
    "profiling_coverage" "The profiling coverage (SELECT or ALL)"
    ...
    "temp_file_encryption" "Encrypt all temporary files if database is encrypted"
